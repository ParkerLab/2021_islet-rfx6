...
// process fastqc {
// 
// 	publishDir "${params.results}/fastqc/before-trim", mode: 'copy', overwrite: true
// 	time '24h'
//         // memory '12 GB'
// 
// 	input:
...
// 	
// 	publishDir "${params.results}/transformed-barcodes", mode: 'copy'
...
// 	
// 	publishDir "${params.results}/corrected-barcodes", mode: 'copy', overwrite: true
// 	tag "${library}"
// 	cpus 3
// 	memory '15 GB'
// 
...
// process trim {
// 
// 	publishDir "${params.results}/trim", mode: 'copy', overwrite: true
// 	errorStrategy 'retry'
// 	maxRetries 1
// 	time '24h'
// 	tag "${library}-${readgroup}"
...
// 
// 	publishDir "${params.results}/fastqc/after-trim", mode: 'copy', overwrite: true
// 	time '24h'
// 	// memory '12 GB'
// 
// 	input:
...
// 
// 	publishDir "${params.results}/bwa", mode: 'copy', overwrite: true
// 
// 	input:
...
// 
// 	tag "${library}-${readgroup}-${genome}"
// 	publishDir "${params.results}/bwa-corrected-barcodes", mode: 'copy', overwrite: true
...
// 
// 	publishDir "${params.results}/species-specific-mapping"
// 
// 	input:
...
// 
// 	time '24h'
// 	publishDir "${params.results}/merge", mode: 'copy', overwrite: true
// 	
// 	input:
...
// process prune {
// 
// 	publishDir "${params.results}/prune", mode: 'copy', overwrite: true
// 	memory '3 GB'
// 	time '5h'
// 	errorStrategy 'retry'
// 	maxRetries 2
...
for (library in libraries) {
	for(genome in get_genome(library)){ 
		tmp << [library, genome, file("/lab/work/vivekrai/2021-06_pilot-rfx6/work/atacseq/mark-duplicates/${library}-${genome}.md.bam"), file("/lab/work/vivekrai/2021-06_pilot-rfx6/work/atacseq/mark-duplicates/${library}-${genome}.md.bam.bai")]
	}	
}

ataqv_in_1 = Channel.from(tmp)
ataqv_in_2 = Channel.from(tmp)

process ataqv {
	
	publishDir "${params.results}/ataqv", mode: 'copy', overwrite: true
	errorStrategy 'retry'
	maxRetries 1
...
process ataqv_custom_ref {
 ...
 process get_ataqv_metrics {
         ...
         memory '100 GB'
         publishDir "${params.results}/ataqv", mode: 'copy'
         cache 'lenient'
         maxForks 2
...
// 	cache false
// 
// 	publishDir "${params.results}/gene-counts", mode: 'copy', overwrite: true
// 
// 	input:
