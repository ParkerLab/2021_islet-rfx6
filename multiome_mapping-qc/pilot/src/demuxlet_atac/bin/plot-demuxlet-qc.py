...
def assemble(f):
    ...
    q = pandas.read_csv(os.path.join(args.qc_dir, f'{library}-hg19.ataqv.txt'), sep='\t',
                        header=None, names=['library', 'barcode', 'metric', 'value'])
    ...
    t = pandas.merge(t, d[['barcode', 'hqaa', 'tss_enrichment', 'fraction_mitochondrial']], how="inner", on="barcode")
...
if __name__ == '__main__':
    ...
    sns.scatterplot(data=d, x="log_hqaa", y="log_mito", hue='out', **{'alpha':0.1}, hue_order = ['DBL','SNG','AMB'])
    plt.xlabel('log10(HQAA)')
    plt.ylabel('log10(fraction mitochondrial)')
    pe.save("fig.demuxlet_qc_mito.png")

    plt.figure(figsize=(5, 4))
    sns.scatterplot(data=d, x="log_hqaa", y="log_tss", hue='out', **{'alpha':0.1}, hue_order = ['DBL','SNG','AMB'])
    plt.xlabel('log10(HQAA)')
