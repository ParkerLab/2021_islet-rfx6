...
if __name__ == "__main__":
    ...
    #     args.weights, sep="\t", usecols=["batch", "sample", "Tissue amount"]
    # )
    # info["sample"] = info["sample"].map(
    #     lambda x: x.replace("M", "") if x.startswith("M") else x
    # )
    # info = info[info["batch"] == args.batch]
    # info["Tissue amount"] = info["Tissue amount"].map(
    #     lambda x: float(str(x).replace(" mg", ""))
    # )
