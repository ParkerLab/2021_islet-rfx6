...
with gzip.open(args.fastq, 'rt') as f:
    for line in f:
        ...
        record_count += 1
        line = line.rstrip()
        for i in range(0, len(line) - 16 + 1):
            potential_barcode = line[i:(i+16)]
            #barcode_counts[line] += 1
            in_whitelist_count += 1 if potential_barcode in whitelist_barcodes else 0
            in_whitelist_rc_count += 1 if potential_barcode in whitelist_barcodes_rc else 0
...
#for k, v in sorted(barcode_counts.items(), key=lambda item: item[1]):
#    print(k, v)
