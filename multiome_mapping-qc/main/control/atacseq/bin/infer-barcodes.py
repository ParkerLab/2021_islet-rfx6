...
whitelist_barcodes_first = set()
whitelist_barcodes_first_rc = set()
whitelist_barcodes_last = set()
whitelist_barcodes_last_rc = set()
...
in_whitelist_first_count = 0
in_whitelist_first_rc_count = 0
in_whitelist_last_count = 0
in_whitelist_last_rc_count = 0
with gzip.open(args.fastq, 'rt') as f:
    for line in f:
        ...
        if len(line) > 16:
            line = line[-16:]
        in_whitelist_count += 1 if line in whitelist_barcodes else 0
        in_whitelist_rc_count += 1 if line in whitelist_barcodes_rc else 0
        #in_whitelist_first_count += 1 if line in whitelist_barcodes_first else 0
        #in_whitelist_first_rc_count += 1 if line in whitelist_barcodes_first_rc else 0
        #in_whitelist_last_count += 1 if line in whitelist_barcodes_last else 0
        #in_whitelist_last_rc_count += 1 if line in whitelist_barcodes_last_rc else 0
        ...
        if record_count % 1000000 == 0:
            ...
            #percent_in_whitelist_first = round(100*float(in_whitelist_first_count) / record_count, 2)
            #percent_in_whitelist_first_rc = round(100*float(in_whitelist_first_rc_count) / record_count, 2)
            #percent_in_whitelist_last = round(100*float(in_whitelist_last_count) / record_count, 2)
            #percent_in_whitelist_last_rc = round(100*float(in_whitelist_last_rc_count) / record_count, 2)
            ...
            #logging.info('Processed {} records so far. {} ({}%) in whitelist_first, {} ({}%) in whitelist_first reverse complemented'.format(record_count, in_whitelist_first_count,  percent_in_whitelist_first, in_whitelist_first_rc_count, percent_in_whitelist_first_rc))
            #logging.info('Processed {} records so far. {} ({}%) in whitelist_last, {} ({}%) in whitelist_last reverse complemented'.format(record_count, in_whitelist_last_count,  percent_in_whitelist_last, in_whitelist_last_rc_count, percent_in_whitelist_last_rc))
